...
public class Scanner
{
	...
	private boolean lookingForStart = false;
	...
	public void lookingForStart(boolean isLooking)
	{
		lookingForStart = isLooking;
	}
		...
		for (int j = 0; j < h; j++)
		{
			...
			for (int i = 0; i < w; i++)
			{
				...
				case 3:
					...
					{
						int mask;
						if (b1 >= 2 && b2 >= 2 && // less than 2 pixels... not
													// interested
						b1 <= maxu && b2 <= maxu && w1 <= (maxu + maxu) && Math.abs(b1 + b2 - w1) <= (b1 + b2) && Math.abs(b1 + b2 - w1) <= w1 && Math.abs(b1 - b2) <= b1 && Math.abs(b1 - b2) <= b2)
						{
							mask = 0x2000000;
	...
	//				{
	//					if ((data[k - 1] & 0x2000000) > 0 && (data[k + 1] & 0x2000000) > 0 && (data[k - h] & 0x2000000) > 0 && (data[k + h] & 0x2000000) > 0)
	//					{
	//						/*
	//						 * if ((data[k-w] & 0x2000000) > 0 || (data[k+w] &
	//						 * 0x2000000) > 0)) {
	//						 */
	//						if (!overlaps(spots, j, i))
	//						{
	//							this.tcount++;
	//							spot.decode(this, j, i);
	//							if (spot.isValid())
	//							{
	//								spots.add(spot);
	//								spot = new TopCode();
	...
	protected List<TopCode> findCodes()
	{
		...
		for (int j = 2; j < h - 2; j++)
		{
			for (int i = 0; i < w; i++)
			{
				if ((data[k] & 0x2000000) > 0)
				{
					if ((data[k - 1] & 0x2000000) > 0 && (data[k + 1] & 0x2000000) > 0 && (data[k - w] & 0x2000000) > 0 && (data[k + w] & 0x2000000) > 0)
					{
						/*
						 * if ((data[k-w] & 0x2000000) > 0 || (data[k+w] &
						 * 0x2000000) > 0)) {
	...
	protected boolean overlapsStart(TopCode s, int x, int y)
	{
		if (s.inBullsEye(x, y))
			return true;
		return false;
	...
	/**
	 * Counts the number of vertical pixels from (x,y) until a color change is
	 * perceived.
	 */
	...
	/**
	 * Counts the number of horizontal pixels from (x,y) until a color change is
	 * perceived.
	 */
