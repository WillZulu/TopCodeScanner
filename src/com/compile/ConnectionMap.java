...
/**
 * This class facilitates the process of pairing sockets and connectors of
 * statements in a tangible program.
 * 
 * @author Michael Horn
...
public class ConnectionMap
{
	...
	/**
	 * Adds a socket location to the connection map and links it back to its
	...
	/**
	 * Adds a connector location to the connection map and links it back to its
	 * originating statement.
	 */
	public void addConnector(Statement statement, String name, double dx, double dy)
	{
		TopCode top = statement.getTopCode();
		if (top == null)
		{
	...
	/**
	 * Uses an inefficient (O(n^2)) algorithm to find and connect all sockets
	 * and connectors that overlap in the digital image.
	 */
		...
		for (int j = 0; j < connectors.size(); j++)
		{
			...
			for (int i = 0; i < sockets.size(); i++)
			{
				...
				if (!c.attached && s.isConnectedTo(c))
				{
					...
					Log.i("HI", "attached to the nextPiece = " + c.attached + " connected = " + s.isConnectedTo(c));
					break;
				}
			}
		}
	...
	/**
	 * For debugging. Outlines all registered sockets and connectors in an
	 * image.
	 */
	// -------------------------------------------------------
	// Inner class: TSocket
	...
	class TConnector
	{
		...
		public TConnector(Statement statement, String name, double x, double y)
		{
